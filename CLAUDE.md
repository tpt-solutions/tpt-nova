...
Phases 0 and 1 are complete, Phase 2 (2D "vibe" sandbox + playable-game basics) is implemented, and Phase 3 (3D cinematic core + smart mesh ingestion) is implemented. The workspace builds, and `cargo fmt --check`, `cargo clippy -D warnings`, and `cargo test` are all green (CI runs this matrix on Windows/Linux/macOS via `.github/workflows/ci.yml`). Implemented crates: `nova-ecs` (ECS core + scene graph + serde + virtual-camera and light components), `nova-telemetry` (JSON + MessagePack sinks), `nova-render` (wgpu cube pipeline, 2D sprite batching/atlas pipeline, and a forward PBR pipeline with a shadow-casting directional light in `pbr.rs`), `nova-input`, `nova-app` (winit shell), `nova-physics` (Rapier2D components + deterministic sync step), `nova-scene` (RON/JSON save/load with versioned migrations), `nova-audio` (SFX/music/bus mixing over rodio), `nova-scripting` (hot-reloadable gameplay dylibs over a C ABI) with `nova-gameplay-example` (a player controller), `nova-ui` (immediate-mode widgets → draw list, plus world-space anchored widgets), `nova-editor` (hierarchy/inspector/2D+3D gizmos + the Bézier "Vibe GUI" bound live to physics gravity), `nova-anim` (skeletal skinning, keyframe sampling, pose blending, and an animation state machine), `nova-ingest` (.glb/.obj loading, VHACD convex decomposition, procedural auto-rig, and Rapier3D collider generation, with an `ingest_demo` binary end-to-end demo), `nova-neural-materials` (live video-LLM texture feeds: a transport-agnostic `FrameSource`/`NeuralMaterialProvider` contract plus `NeuralTexture` GPU upload and a `NeuralMaterialRegistry` ECS resource, with a network-free `MockProvider` for tests/demos), and `nova-scripting-embedded` (sandboxed Rhai scripting with a capability boundary: scripts call only the host-registered, capability-gated functions and enqueue typed `ScriptCommand`s the host applies to the `World`). Always check `todo.md` for the authoritative phase status before assuming a system's maturity.
...
- **Planned crate boundaries** (per `todo.md`; not all exist yet):
  ...
  - `nova-app` — the application/window shell (winit + wgpu device/surface setup, main loop).
      - Later phases introduce further crates as needed: `nova-input`, `nova-audio`, `nova-scripting` (hot-reloadable gameplay dylibs), `nova-ui`, `nova-anim`, `nova-ingest` (mesh/.glb ingestion, VHACD colliders, auto-rigging), `nova-neural-materials` (live video-LLM texture feeds), `nova-scripting-embedded` (sandboxed Rhai scripting), `nova-export`, `nova-rag`. Don't create these speculatively — add them when the corresponding `todo.md` phase is actually being worked on.
